...
const msg = {
  ...
  body: [
    'god，DSH 12:06Z 实测：Ryan 会话 11:55:21 起停摆（3411B 小会话），t-151/m2v197 派单（11:54:47 送达）后连续 2 次模型调用失败：错误=『API key reached its budget (pollinations.ai key <key-id 已脱敏：向 god/告警原文索取>)』，resp=gpt-oss——与昨晚 22:19Z 同款（tuan 池 gpt-oss 路由到 pollinations 且预算耗尽）。',
    'Ryan 配置=仅 tuan provider（127.0.0.1:8790，多模型网关），无备用 provider；现役模型路由到 gpt-oss=撞预算。',
    '处置建议（参考 22:2xZ 先例）：a) 换 Ryan 模型通道（如 god 席现用 deepseek-v4-flash 或 tuan 池内其他未受影响模型）；b) 或 app 重启六席归位（若其他席也受影响）；c) pollinations key <KEY_ID_脱敏> 提升预算=人类边界（前次已判暂不升 ASK ME，若这次阻塞 t-151 主线请再裁）。',
