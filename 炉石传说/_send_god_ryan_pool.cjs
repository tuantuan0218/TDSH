...
const msg = {
  ...
  body: [
    ...
    '1) Ryan 最新会话 22:19Z 起每次模型调用即失败：错误="The API key used for this request has reached its budget. Please raise the key budget (pollinations.ai key id=<key-id 已脱敏：向 god/告警原文索取>)"（resp=gpt-oss）；Topping up wallet 不解锁，只提升 key budget 有效。',
    ...
    '建议（供裁决）：a) 给 Ryan 换 provider/model 配置（参考 god 席 deepseek-v4-flash 通道，或 models.json 内其他可达模型）→ 恢复后即可构建；b) 若无法远程改配置，持久恢复依赖 pollinations key budget 提升（人类边界，key id <KEY_ID_脱敏>）或 tuan 池渠道修复——可挂 ASK ME 转用户；c) DSH 侧可代为本地构建（mvn 在 D:/tdsh/hs_bridge_build/drawfix，_build_drawfix.ps1 现成）但按换栈铁律仍建议 Ryan 出件+你三查，仅应急可代跑。',
