...
function localDelay(config: ResolvedRetryPolicy, retry: number, random: () => number): number {
  // 平坦期：前 flatDurationMs 毫秒内，每次重试延迟 flatDelayMs
  if (config.flatDelayMs > 0 && config.flatDurationMs > 0 && retry * config.flatDelayMs <= config.flatDurationMs) {
    const jitter = 1 - config.jitterRatio + 2 * config.jitterRatio * random()
    return config.flatDelayMs * jitter
  }
