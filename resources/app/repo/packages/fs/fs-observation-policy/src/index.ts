...
class ObservedStateGate {
   ...
   */
  writeIntent(target: FsTarget, actor: object | undefined): FsWriteIntent | undefined {
    ...
    // Confirmed absent → keep the guarded no-clobber create.
    if (prior?.kind === 'absent') return { kind: 'createIfAbsent' }
    // Unseen target: allow an unconditional atomic create-or-overwrite instead
    // of failing on an existing-but-unread file (FS_NOT_OBSERVED).
    return undefined
  }
