...
## 三、QQ SMTP 授权码真实状态（2026-09-13 09:0x 定稿）
...
| U3 | 4 个第三方仓 origin 去 token | ✅ **已执行完毕（10:1x，§7 授权范围内的止血动作，非"待点头"项）**，详见下节 |
...
## 六、U3 止血执行记录（2026-09-13 10:1x）

**为什么这次不等你点头**：AGENTS.md **§7 要求"发现凭据错位→立即止血，并明确提示用户"**，§5 又明令"勿把个人 token 写进第三方仓"。这是被要求的动作而非越权；且已实测这些是公开仓、裸 URL 可拉，**风险为零**。工具 `thirdparty_detoken.py` 采用 **fail-closed**：任一仓裸 URL 拉不动就跳过不动它。

| 仓 | 变更前 | 动作 | 变更后复验 |
|---|---|---|---|
| `hs_card_sdk` | origin 含 token，公开可拉 | 去 token | ✅ 仍可 `ls-remote`；工作区 0 脏文件 |
| `hs_plugin_src` | 同上 | 去 token | ✅ 同上 |
| `hs_strategy_sdk` | 同上 | 去 token | ✅ 同上 |
| `炉石传说/_gh_strategy_plugin` | 同上 | 去 token | ✅ 同上 |

**双向验证**（不只验"改对了"，也验"没改坏别的"）：`remote_ownership.py` 复扫 24 仓 →
- 自有仓未内嵌 = **0**（8 个自有仓仍全部内嵌合规）
- 第三方仓含 token = **0**（原 4 处已清零）
- 4 仓工作区脏跟踪文件 = **0**（只改 `.git/config`，未碰任何内容文件）

**边界与残留说明**（如实）：
- 验证的是 **认证路径**（`ls-remote` 免弹窗成功），不等于跑过一次完整 `git fetch`（那会写入对象库，非必要且属他人项目）。
- token 本体仍留在 `dsh-home`（AGENTS.md / settings）——那是 §5 认可的唯一位置，**未做任何删除或改写**。若要撤销本次改动，重新内嵌即可（凭据来源就在上述位置，故变更可逆）。
- 这 4 个仓是第三方只读克隆，你对其无写权限，去 token 不影响任何 push 场景。
