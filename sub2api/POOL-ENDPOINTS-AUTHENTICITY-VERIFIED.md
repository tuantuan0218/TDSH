...
## 三、pollinations 的问题：**预算分钟级波动**（⚠️ 本节结论已被后续实测修正）

> ⚠️ **2026-09-13 后续修正**：本文当时把 pollinations 判为"预算耗尽"，
> **该判定在那一刻成立，但不宜作为长期定性**。
> 深挖后发现其预算状态**在分钟级波动**：同一 prompt 可能此刻成功、下一刻返回预算文本。
> 详见 **`POLLINATIONS-BUDGET-FLUCTUATION.md`**。
> 准确表述应为：**"真实端点、预算受限、分钟级波动、仍可间歇使用"**。
