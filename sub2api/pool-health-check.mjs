 ...
 */
{
  ...
  if (r.error) log(`⚠️ 上下文能力查询失败：${r.error}`);
    ...
    log('  > 2026-09-13 机制查明：请求按**名次从前到后**选择账号 → 排在前面但天花板低的号');
    log('    （如 #3 agenes 第3位/490K）会先接到超长请求并必然失败；');
    log('    排得靠后的低天花板号（如 #16 tele-qwen 第8位/393K）因长请求早被前排消费 → 反而零错误。');
    log('    ⇒ 关键变量是**名次位置 × 上下文能力**的组合，不是单一账号属性。');
  }
}
