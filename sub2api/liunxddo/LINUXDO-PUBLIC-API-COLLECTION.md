...
### 2026 年新公益站（补充收集，2026-09-13 补录）
- **V-API**（2026-07 上线，linux.do/t/topic/2518225）：**实测（2026-09-13）域名
  `v-api.de5.net` 活**——`/api/status` 200（3178B 状态页可达）、`/v1/models` 401
  Invalid token（New API 系需 key，端点正常）；首页 TLS 偶发被断（CF 防护正常现象）。
  New API 架构，聚合 DeepSeek/GLM 等；LINUX DO Connect 登录；CDK 商店（LD士多
  ldcstore.com/product/9481）10 刀 CDK；每日签到改关闭，LDC 积分兑换（10:1）；Claude
  不定期开放。**受限项：需 linux.do 账号 Connect 领额度**
- **魔方公益站**（2026-07 上线，linux.do/t/topic/2426925）：域名在帖内为图片未能提取
  （抓取不可见）；实测信息：Connect 鉴权、默认 0 额度每日签到 10-100、LDC 充值 10:1、
  RPM 10 并发（1 分钟≤1000 请求）、禁分发 key、严格法律免责（禁传敏感信息）；
  **受限项：需 linux.do 账号**
- **汐洛公益**（2026-04 上线，linux.do/t/topic/1902637）：原帖已失效（404/私密），
...
- 与 hub.linux.do（闲置置换 marketplace）相比：V-API/魔方/汐洛是"聚合公共号池"型，
  稳定性依赖捐赠，仅适合测试；hub 是 marketplace 型，更可持续——**推荐序不变：hub 优先**
...
- **DeepLX 本地 API（111602）**：✅ **已实际部署**（`liunxddo/bin/deeplx_windows_amd64.exe`，
  ...
  - **最终结论（2026-09-13 13:0x，4 次间隔 15s + 冷却 90s 全部 429）**：DeepL 对当前出口 IP
    （Clash 代理节点共享 IP）持续限流，非临时抖动。DeepLX 服务本身 100% 正常（0.0.0.0:1188
    协议层响应正确），但**翻译上游可用性取决于出口 IP 干净度**——代理环境下大概率 429；
    直连/家庭宽带出口预计正常（DeepLX 设计场景）。**部署评估：服务就绪，生产可用需干净出口**。
  - **二次复核（2026-09-13 13:1x，距首次 429 超 1 小时）**：进程存活（20.8MB），翻译仍 429——
    **确认本机代理出口 IP 的 DeepL 限流为持续状态**（小时级未解），非临时抖动。结论不变：
    服务就绪、需干净出口；作为交付记录"代理环境下不可用，直连可试"。
  - **三次复核（2026-09-13 14:2x，距首次 429 超 3 小时）**：仍 429——**确认 DeepL 对
    当前出口 IP 为长期拉黑**（小时级以上），修复仅剩换出口 IP（Clash 换节点/直连）这一
    用户侧动作。DeepLX 服务本身始终正常（监听 1188，协议响应正确）。
  - **四次复核（2026-09-13 14:5x，距首次超 4 小时）**：仍 429——拉黑为持续态（小时级+
    未解），**结论固化：代理出口不可用，需用户换出口后方可验证**；不再重复探测（免费
    权益：避免把 429 当新现象误报）。
