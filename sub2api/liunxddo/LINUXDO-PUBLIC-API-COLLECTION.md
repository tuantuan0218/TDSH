...
- **V-API**（2026-07 上线）：New API 架构统一网关，聚合 DeepSeek/GLM 等；接入 LINUX DO
  Connect 登录；首发 100 份 $10 兑换码（CDK）限时领取 + 每日签到随机金额 + LDC 积分 1:5 兑换；
  非营利无广告。入口需在 linux.do 内找官方帖（80aj 报道源已存档）
- **魔方公益站**（2026-07 上线）：个人非营利聚合网关，统一 OpenAI/Claude/Gemini 调用接口；
  Connect 鉴权 + 每日签到 10-100 额度 + LDC 兑换；**RPM 10 并发限制**，部分模型 503（上游负载）；
  法律免责声明明确：数据会传至上游第三方，**禁传敏感信息**，仅适测试沙盒
- **汐洛公益**（2026-04 上线）：OpenAI 兼容格式，支持 GPT/Grok；邀请码 + 签到分配额度；
  公开招募捐赠 Key 壮大公益号池；稳定性依赖资源供给，适合轻度尝鲜非商业
